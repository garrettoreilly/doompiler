...
parseProgram :: [Token] -> String
parseProgram xs = case findBlock xs of
                      [Token EOF _ _ _] -> "Compiled successfully!"
                      (Token Error a b c : ys) -> error $ a ++ show (kind (head ys)) ++ "\nLine " ++ show (line (head ys)) ++ ", position " ++ show (position (head ys))
                      (y:ys) -> error $ "Error! Invalid token, received " ++ show (kind y) ++ "\nLine " ++ show (line y) ++ ", position " ++ show (position y)
